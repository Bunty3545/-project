...
<head>
    ...
    <style>
        ...
        /* This is the green progress line */
        .checkout-steps::after {
            ...
            height: 2px;
            background-color: #10B981; /* Green color for completed progress */
            z-index: 1;
            transition: width 0.5s ease;
        }
        
        /* When all steps are completed, extend the green line fully */
        .checkout-steps.all-completed::after {
            width: 100%;
        }
...
<body class="bg-gray-50">
    ...
    <div class="container px-4 py-8 mx-auto max-w-7xl">
        ...
        <!-- Checkout Steps -->
          <!-- Checkout Steps -->
          <div class="mb-8 checkout-steps" id="checkoutStepsContainer">
          <div class="step <?= ($_SESSION['checkout_step'] == 'cart' ? 'active' : ($_SESSION['checkout_step'] == 'shipping' || $_SESSION['checkout_step'] == 'payment' || $_SESSION['checkout_step'] == 'confirmation' ? 'completed' : '')) ?>" id="cartStep">
            ...
            <div class="step <?= ($_SESSION['checkout_step'] == 'confirmation' ? 'active' : '') ?>" id="confirmStep">
                <div class="step-circle" id="confirmStepCircle">4</div>
        ...
        <?php if(empty($cart_items)): ?>
            ...
            <div class="p-8 my-4 text-center bg-white rounded-lg shadow-md">
                ...
                <a href="index.php" class="inline-block px-6 py-3 text-white transition-all duration-300 rounded-full btn-animated bg-primary hover:bg-secondary">
    ...
    <script>
        ...
        function updateProgressLine() {
            ...
            // Calculate the width percentage for the green line
            const progressPercentage = (completedSteps / steps.length) * 100;
            
            // Apply the width directly to the progress line
            const checkoutSteps = document.querySelector('.checkout-steps');
            if (checkoutSteps) {
                checkoutSteps.style.setProperty('--progress-width', `${progressPercentage}%`);
                
                // If all steps are completed or confirmation is active, add a special class
                if (document.getElementById('confirmStep').classList.contains('active')) {
                    checkoutSteps.classList.add('all-completed');
                    checkoutSteps.style.setProperty('--progress-width', '100%');
                }
            }
        }
        ...
        function goToConfirmation() {
            ...
            // Make sure the progress bar extends to 100%
            const checkoutSteps = document.querySelector('.checkout-steps');
            if (checkoutSteps) {
                checkoutSteps.classList.add('all-completed');
                checkoutSteps.style.setProperty('--progress-width', '100%');
                ...
                confirmationSection.innerHTML = `
                    <div class="p-6 mb-6 text-center bg-white rounded-lg shadow-md">
                        ...
                        <div class="flex flex-col justify-center gap-4">
                            ...
                            <button id="confirm-order-button" onclick="handleOrderConfirmation()" class="confirmation-button">
        ...
        // Function to specifically handle confirmation order button click
        function handleOrderConfirmation() {
            ...
            // Force confirmation step to be completed (green with checkmark)
            forceConfirmationStepGreen();
            ...
            .then(data => {
                ...
                continueShoppingButton.href = 'clothing.php';
                continueShoppingButton.className = 'continue-shopping-button';
                continueShoppingButton.innerHTML = '<i class="fas fa-shopping-bag"></i> Continue Shopping';
                continueShoppingButton.style.display = 'block';
                continueShoppingButton.style.width = '100%';
                continueShoppingButton.style.padding = '12px 20px';
                continueShoppingButton.style.backgroundColor = '#3B82F6';
                continueShoppingButton.style.color = 'white';
                continueShoppingButton.style.border = 'none';
                continueShoppingButton.style.borderRadius = '6px';
                continueShoppingButton.style.fontWeight = '600';
                continueShoppingButton.style.cursor = 'pointer';
                continueShoppingButton.style.transition = 'all 0.3s ease';
                continueShoppingButton.style.textAlign = 'center';
                continueShoppingButton.style.marginTop = '20px';
                continueShoppingButton.style.textDecoration = 'none';
                
                // Replace confirmation button with continue shopping button
                confirmButton.parentNode.replaceChild(continueShoppingButton, confirmButton);
                ...
                if (confirmationSection) {
                    ...
                    successMessage.innerHTML = '<i class="mr-2 fas fa-check-circle"></i> Order confirmed! Hope you have a amazing experience with us.';
                    
                    // Insert the message before the continue shopping button
                    continueShoppingButton.parentNode.insertBefore(successMessage, continueShoppingButton);
                }
            })
            .catch(error => {
        ...
        // Function to force the confirmation step to be green with checkmark
        function forceConfirmationStepGreen() {
            console.log("Forcing confirmation step to be green");
            
            // Force the confirmation step to be completed (green with checkmark)
            const confirmStep = document.getElementById('confirmStep');
            const confirmCircle = document.getElementById('confirmStepCircle');
            
            if (confirmStep && confirmCircle) {
                // Remove active class and add completed
                confirmStep.className = 'step completed';
                
                // Replace number with checkmark and apply green styling
                confirmCircle.innerHTML = '<i class="fas fa-check"></i>';
                confirmCircle.style.backgroundColor = '#10B981';
                confirmCircle.style.color = 'white';
                
                // Apply !important styles to ensure it overrides any other styles
                const style = document.createElement('style');
                style.textContent = `
                    #confirmStep .step-circle {
                        background-color: #10B981 !important;
                        color: white !important;
                    }
                    #confirmStep .step-label {
                ...
                // Ensure the progress line is 100%
                const checkoutSteps = document.querySelector('.checkout-steps');
                if (checkoutSteps) {
                    checkoutSteps.classList.add('all-completed');
                    checkoutSteps.style.setProperty('--progress-width', '100%');
